...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  // Check for AI Gateway or direct Google configuration
  if (env.AI_GATEWAY_API_KEY) {
    ...
    // Direct Google access requires API key
    missing.push('GOOGLE_API_KEY or AI_GATEWAY_API_KEY');
  }
...
app.use('*', async (c, next) => {
  ...
  console.log(`[REQ] Has GOOGLE_API_KEY: ${!!c.env.GOOGLE_API_KEY}`);
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.GOOGLE_API_KEY) {
      hint = 'GOOGLE_API_KEY is not set. Run: wrangler secret put GOOGLE_API_KEY';
